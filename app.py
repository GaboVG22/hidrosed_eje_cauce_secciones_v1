...
# App UI
# --------------------------------------------------------------------------------------
st.title("🌊 HidroSed · Módulo Eje Cauce y Secciones")
st.caption("Etapa 1: eje útil y sentido hidráulico · Etapa 2: secciones · Etapa 3: visualización 3D y exportación")

with st.expander("Criterio técnico del módulo", expanded=False):
    st.markdown(
        """
Este módulo trabaja **después de contar con puntos de control y eje de cauce**. Su foco no es delimitar cuencas, sino construir el insumo hidráulico:
...
- exportación KMZ, Excel tipo HEC-RAS, CSV de perfil y JSON técnico.

La aplicación es conservadora: no habilita secciones naturales si no existe DEM, y marca advertencias cuando el eje, el perfil o las secciones son incoherentes.
"""
    )
...
st.sidebar.header("2. Entradas opcionales")
dem_file = st.sidebar.file_uploader("DEM GeoTIFF opcional para perfil/secciones naturales", type=["tif", "tiff"])
...
st.sidebar.header("3. Parámetros eje")
...
st.sidebar.header("4. Parámetros secciones")
section_mode = st.sidebar.selectbox("Modo de generación de secciones", ["Desde DEM natural", "Prismática por parámetros", "Desde Excel prismático"])
...
if not run:
    st.info("Carga el eje del cauce, PC hidrológico y PC cuenca soporte. Luego presiona **Procesar eje y secciones**.")
    st.stop()
...
try:
    ...
    # DEM opcional
    dem = DemSampler(dem_file) if dem_file is not None else None
    ...
    if dem is None and section_mode == "Desde DEM natural":
        warnings.append("Modo natural seleccionado sin DEM. No se generarán secciones naturales.")

    # Curvas apoyo opcionales
    curves = load_support_curves(curves_file, to_proj) if curves_file is not None else []

    # Secciones
    if section_mode == "Desde DEM natural":
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Etapa 1 · Eje", "Etapa 2 · Secciones", "Etapa 3 · 3D / Corrección", "Descargas"])
    ...
    with tab1:
        ...
        col3.metric("PC hidrológico sobre eje", f"km {d_h/1000:.3f}")
        col4.metric("PC soporte sobre eje", f"km {d_s/1000:.3f}")
        st.write(direction_note)
        ...
        st.dataframe(profile_df.head(200), use_container_width=True)
    ...
    with tab2:
        ...
        if not sections:
            ...
            c3.metric("Revisar", sum(1 for s in sections if s.status != "aceptada"))
            st.write(f"**Evaluación curvas de apoyo:** {curve_eval['estado']} · {curve_eval['detalle']}")
            idx = st.slider("Seleccionar sección", 0, max(0, len(sections)-1), 0)
            sec = sections[idx]
            st.write(f"Sección km {sec.km:.3f} · fuente: {sec.source} · estado: {sec.status}")
            fig, ax = plt.subplots(figsize=(8, 4))
            ax.plot(sec.stations, sec.elevations, marker="o")
            ax.set_xlabel("Station [m] izquierda → derecha mirando aguas abajo")
            ax.set_ylabel("Cota [m]")
            ax.set_title(f"Sección km {sec.km:.3f}")
            ax.grid(True, alpha=0.3)
            st.pyplot(fig)
    ...
    with tab3:
        st.subheader("Visualización 3D referencial")
        fig3d = plot_sections_3d(axis_useful, sections)
        ...
        st.markdown(
            """
Esta versión v1 permite revisar visualmente las secciones y exportarlas. La edición interactiva fina se deja preparada para v2:

- aceptar/rechazar sección;
- suavizar sección;
- mover sección;
- editar fondo, taludes o ancho;
- interpolar entre secciones válidas.
"""
        )
    ...
    with tab4:
        ...
        st.download_button("Descargar KMZ eje + secciones", kmz_bytes, file_name="hidrosed_eje_cauce_secciones.kmz", mime="application/vnd.google-earth.kmz")
        ...
        st.download_button("Descargar Excel tipo HEC-RAS", sections_to_hecras_excel(sections), file_name="secciones_tipo_hecras.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        summary = {
            "modulo": "HidroSed Eje Cauce y Secciones v1",
            "crs_proyectado": crs_proj.to_string(),
            "axis_validation": asdict(validation),
            "pc_hidrologico_original_lonlat": [pc_h_lon, pc_h_lat],
            "pc_soporte_original_lonlat": [pc_s_lon, pc_s_lat],
            "dist_pc_hidrologico_a_eje_m": dist_pc_h,
            "dist_pc_soporte_a_eje_m": dist_pc_s,
            "numero_secciones": len(sections),
            "evaluacion_curvas_apoyo": curve_eval,
            "modo_secciones": section_mode,
        }
        st.download_button("Descargar JSON técnico", json.dumps(summary, indent=2, ensure_ascii=False).encode("utf-8"), file_name="resumen_tecnico_eje_secciones.json", mime="application/json")
